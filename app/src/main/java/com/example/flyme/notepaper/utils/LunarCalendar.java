package com.example.flyme.notepaper.utils;
...
public class LunarCalendar {
    ...
    public static final int[] lunarToSolar(int year, int month, int monthDay, boolean isLeapMonth) {
        ...
        for (i = 1; i < month; i++) {
            if ((LUNAR_INFO[year - 1900] & (AccessibilityEventCompat.TYPE_GESTURE_DETECTION_END >> (i - 1))) == 0) {
                dayOffset += 29;
            } else {
                dayOffset += 30;
            }
        ...
        if (leapMonth != 0 && (month > leapMonth || (month == leapMonth && isLeapMonth))) {
            dayOffset = (LUNAR_INFO[year + -1900] & (AccessibilityEventCompat.TYPE_GESTURE_DETECTION_END >> (month + -1))) == 0 ? dayOffset + 29 : dayOffset + 30;
        }
    ...
    private static int daysInLunarYear(int year) {
        ...
        for (int i = AccessibilityEventCompat.TYPE_GESTURE_DETECTION_END; i > 7; i >>= 1) {
            if ((monthInfo & i) != 0) {
                sum++;
            }
        }
    ...
    private static int daysInLunarMonth(int year, int month) {
        if ((LUNAR_INFO[year - 1900] & (AccessibilityEventCompat.TYPE_TOUCH_INTERACTION_START >> month)) == 0) {
            return 29;
        }
        return 30;
    }
    ...
    public static final int[] parseLunarDate(String date) {
        ...
        throw new InvalidParameterException("Invalid date : " + date + " to parse. Must be from 1900-1-1 to 2099-12-31 in format YYYY-MM-DD");
    }
}
