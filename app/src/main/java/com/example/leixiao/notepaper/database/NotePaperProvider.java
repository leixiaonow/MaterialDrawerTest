...
import com.example.leixiao.notepaper.TagData;
import com.example.leixiao.notepaper.database.NotePaper.AccountConstract;
import com.example.leixiao.notepaper.database.NotePaper.NoteCategory;
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        switch (match) {
            case NOTES /*1*/:
                ...
                //        break;
                //    }
                qb.appendWhere(Notes.ENCRYPT + " IS NULL" + " OR " + Notes.ENCRYPT + "<>" + NOTES + " OR " + "(" + Notes.ENCRYPT + "=" + NOTES + ")");
...
//                break;
            ...
            case SYNCSTATE_ID /*5*/:
                return this.mOpenHelper.getSyncState().query(this.mOpenHelper.getReadableDatabase(), projection, "_id=" + ContentUris.parseId(uri) + HanziToPinyin.Token.SEPARATOR + (TextUtils.isEmpty(selection) ? BuildConfig.VERSION_NAME : " AND (" + selection + ")"), selectionArgs, sortOrder);
            case FILES /*11*/:
            ...
            case NOTES /*1*/:
                ...
                if (TagData.FUN_ENCRYPT) {
                    if (!values.containsKey(Notes.ENCRYPT)) {
                        ...
                        //如果从NoteApplication中读取到了账户accountID，则put加入账户属性，注释即可
//                        long accountID = ((NoteApplication) getContext().getApplicationContext()).getMeizuAccount();
//                        if (accountID > 0) {
...
//                        }
    ...
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        switch (type) {
            ...
            case SYNCSTATE_ID /*5*/:
                return this.mOpenHelper.getSyncState().delete(db, "_id=" + ContentUris.parseId(uri) + HanziToPinyin.Token.SEPARATOR + (TextUtils.isEmpty(where) ? BuildConfig.VERSION_NAME : " AND (" + where + ")"), whereArgs);
            case FILES /*11*/:
        ...
        switch (type) {
            case NOTES /*1*/:
                if (TagData.FUN_ENCRYPT && values.containsKey(Notes.ENCRYPT) && values.getAsInteger(Notes.ENCRYPT).intValue() == NOTES) {
                    //从NoteApplication读取账户，如果有账户，put到Values中，没有就不加入属性，注释即可
...
//                    }
            ...
            case SYNCSTATE_ID /*5*/:
                return this.mOpenHelper.getSyncState().update(db, values, "_id=" + ContentUris.parseId(uri) + HanziToPinyin.Token.SEPARATOR + (TextUtils.isEmpty(where) ? BuildConfig.VERSION_NAME : " AND (" + where + ")"), whereArgs);
            case FILES /*11*/:
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        while (i$2.hasNext()) {
//          cv = (ContentValues) i$2.next();//原来的
            ContentValues cv=(ContentValues) i$2.next();//更改后的
    ...
    public static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private boolean copydb(int rawFile, String uuid, String name) {
            ...
            try {
                ...
                Log.d(NotePaperProvider.TAG, "create getParentFile fail: " + uuid + HanziToPinyin.Token.SEPARATOR + name);
                if (is != null) {
                    try {
                        is.close();
